...
const anchors = [
  ...
  ["Module 2 · Design", "Design", "Fan out, then decide on paper.", "Read-only exploration returns conclusions, not file dumps; the decision earns a durable ADR — threats included, while the design is still cheap to change.", "Explore / Plan sub-agents · plan mode · ADRs · threat model", "Design the CRC change; threat-model the frame (CRC ≠ tamper-proofing); a design-reviewer sub-agent critiques the ADR.", "L3"],
  ...
  ["Module 7 · Build & Integrate", "Build & integrate", "The first true L4.", "Objective oracle + sandbox + audit log make unattended autonomy genuinely safe here — but a CI agent reads attacker-controlled PRs: scope its tools, keep merge authority human.", "Headless claude -p · build-fixer loop · /code-review · CI bot · injection guardrails · supply chain", "Break the build; an L4 fixer loop repairs it; agent review stacks before human review.", "L4"],
  ["Module 8 · Release", "Release", "Automate the typing, not the call.", "Notes, changelog, and tag checks are delegable; the go/no-go to ship stays human — and what ships is signed.", "release-notes skill · semver hook · signed tag + digests · /schedule", "Cut v1.1.0: the agent drafts notes from commits, you approve and sign the tag.", "L4"],
